...
@interface ThreeCardMonteViewController ()
...
@synthesize cancelInfoButton, infoView;
...
- (IBAction)okay:(id)sender {
  ...
  [standard setObject:[NSNumber numberWithBool:NO] forKey:@"didStart"];
...
- (void)prizeCalculation {
  int prize = arc4random() % 100;
  prize += 1;
  if(prize > 0 && prize <= 60) {
    reward = Smaller;
    cardID = badCardID;
  }
  else if(prize > 60 && prize <= 90){
    reward = Equal;
    cardID = mediumCardID;
  }
  else if(prize > 90 && prize <= 100) {
    reward = Greater;
    cardID = goodCardID;
  }
}
...
- (void)cardOneTouched:(UITapGestureRecognizer *)gesture {
  ...
  [UIView transitionWithView:self.cardImageViewOne duration:Flip_duration
                     options:UIViewAnimationOptionTransitionFlipFromRight animations:^{
                       ...
                       [self performSelector:@selector(performHidden) withObject:nil afterDelay:Hidden_Delay];
...
- (IBAction)backToGame:(id)sender {
  [UIView animateWithDuration:0.3f animations:^ {
    ...
    self.infoView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
...
- (IBAction)info:(id)sender {
  [UIView animateWithDuration:0.3f animations:^{
    ...
    self.infoView.frame = CGRectMake(50, 64, self.view.frame.size.width, self.view.frame.size.height);
...
- (void)notEnoughDiamonds {
  [Globals popupMessage:@"You dont have enough gold"];
}
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self.view insertSubview:self.spinner aboveSubview:self.cardViewOne];
  ...
  self.flipButton.enabled = NO;
  NSNumber *didStart = [[NSUserDefaults standardUserDefaults]objectForKey:@"didStart"];
  [[OutgoingEventController sharedOutgoingEventController] retrieveThreeCardMonte];
  if([didStart intValue] != 0) {
    [self restartView];
    NSUserDefaults *standard = [NSUserDefaults standardUserDefaults];
    [standard setObject:[NSNumber numberWithBool:NO] forKey:@"didStart"];
    [standard synchronize];  
...
-(void)calculateBadCardValues:(RetrieveThreeCardMonteResponseProto *)proto {
  ...
  if(proto.badMonteCard.hasCoinsGained && proto.badMonteCard.hasEquip && proto.badMonteCard.hasDiamondsGained) {
    ...
    self.decentTwoItemTwoAmount.text = [NSString stringWithFormat:@"1"];
    ...
    self.decentWeaponName.text = [NSString stringWithFormat:@"%@",equip.name];
...
-(void)calculateMediumCardValues:(RetrieveThreeCardMonteResponseProto *)proto {
  ...
  if(proto.mediumMonteCard.hasCoinsGained && proto.mediumMonteCard.hasEquip && proto.mediumMonteCard.hasDiamondsGained) {
    ...
    self.betterTwoItemTwoAmount.text = [NSString stringWithFormat:@"1"];
...
-(void)calculateGoodCardValue:(RetrieveThreeCardMonteResponseProto *)proto {
  ...
  if(proto.goodMonteCard.hasCoinsGained && proto.goodMonteCard.hasEquip && proto.goodMonteCard.hasDiamondsGained) {
    ...
    self.bestTwoItemTwoAmount.text = [NSString stringWithFormat:@"1"];
...
- (void) receivedPlayThreeCardMonteResponse:(PlayThreeCardMonteResponseProto *)proto{
  ...
  [defaults setObject:[NSNumber numberWithBool:YES] forKey:@"didStart"];
